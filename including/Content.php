...
class Content
{
    ...
    {
        try {
            $page = new \including\Page($pageId);
        } catch (\including\Exception $e) {
            return null;
        }
        return $page;
     ...
     *
     * @return \content\Language[] All website languages. Each element is a Language object.
     *
     */
    public function getLanguages()
    {
        if ($this->languages === null) {
            if (!knConfig()->database()) {
                ...
                $languages = \content\Internal\Languages\Service::getLanguages();
                $this->languages = array();
                foreach ($languages as $data) {
                    $this->languages[] = \content\Internal\Content\Helper::createLanguage($data);
                }
            }
        }
    ...
    {
        return new \content\Block($blockName);
    }
    ...
    public function getCurrentRevision()
    {
        ...
        if (knRequest()->getQuery('_revision') && knAdminId()) {
            $revisionId = knRequest()->getQuery('_revision');
            $revision = \content\Internal\Revision::getRevision($revisionId);
            if ($revision['pageId'] != $pageId) {
                $revision = null;
            }
        }

        if (!$revision && knIsManagementState()) {
            $revision = \content\Internal\Revision::getLastRevision($pageId);
            if ($revision['isPublished']) {
                $duplicatedId = \content\Internal\Revision::duplicateRevision($revision['revisionId']);
                $revision = \content\Internal\Revision::getRevision($duplicatedId);
            }
        }

        if (!$revision) {
            $revision = \content\Internal\Revision::getPublishedRevision($this->currentPage->getId());
        }
    ...
    public function getBreadcrumb($pageId = null)
    {
        ...
        if ($page) {
            ...
            while (!empty($parentPageId)) {
                $parentPage = new \content\Page($parentPageId);
                $pages[] = $parentPage;
        ...
        $breadcrumb = knFilter('knBreadcrumb', $breadcrumb);
        return $breadcrumb;
    }
    ...
    {
        $languageId = \content\Internal\Languages\Service::addLanguage(
            $title,
            $abbreviation,
            $code,
            $url,
    ...
    {
        \content\Internal\Languages\Service::delete($languageId);
    }
    ...
    {
        \content\Internal\Pages\Service::updatePage($pageId, $data);
    }
    ...
    {
        $newPageId = \content\Internal\Pages\Service::addPage($parentId, $title, $data);
        return $newPageId;
    }
    ...
    {
        $pageId = \content\Internal\Pages\Service::copyPage($pageId, $destinationParentId, $destinationPosition);
        return $pageId;
    }
    ...
    {
        \content\Internal\Pages\Service::movePage($pageId, $destinationParentId, $destinationPosition);
    }
    ...
    {
        \content\Internal\Pages\Service::deletePage($pageId);
    }
     ...
     * Eg. if (knContent()->getPageMenu()->getAlias() == 'menu1') //if current page is one of pages in menu1
     * @param null $pageId
     * @return Page
     */
    ...
    {
        $result = \content\Internal\Pages\Service::getMenus($languageCode);
        $objectArray = array();
        foreach ($result as $menuData)
        {
            $objectArray[] = new \content\Page($menuData);
        }
        return $objectArray;
    }
    ...
    {
        $result = \content\Internal\Pages\Service::getMenu($languageCode, $alias);
        if ($result) {
            return new \content\Page($result);
        }
        return $result;
    }
    ...
    {
        return \content\Internal\Pages\Service::createMenu($languageCode, $alias, $title, $type);
    }
    ...
    public function getDefaultPageId($languageCode = null)
    {
        ...
        $pageId = knJob('knDefaultPageId', array('languageCode' => $languageCode));
        return $pageId;
    }
