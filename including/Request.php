...
class Request
{
    ...
    public function __construct()
    {
        ...
        $server['REDIRECT_URL'] = '';
        $server['QUERY_STRING'] = '';
        $server['REQUEST_URI'] = parse_url(knConfig()->baseUrl(), PHP_URL_PATH); // default uri points to root
     ...
     *
     * @throws \including\Exception is thrown if POST method was not used.
     */
    public function mustBePost()
    {
        if (!$this->isPost()) {
            throw new \including\Exception('POST method required.');
        }
    }
    ...
    {
        $basePath = parse_url(knConfig()->baseUrl(), PHP_URL_PATH);
    ...
    /**
     * @private
     * for internal Kholifa CMS uses only
     */
