...
class Db
{
    ...
    public function getConnection()
    {
        ...
        $dbConfig = knConfig()->get('db');
        knConfig()->set('db', null);

        if (empty($dbConfig)) {
            throw new \including\Exception\Db("Can't connect to database. No connection config found or \\Ip\\Db::disconnect() has been used.");
        }
        ...
        try {
            ...
            throw new \including\Exception($e->getMessage());
    ...
    public function fetchValue($sql, $params = array())
    {
        try {
            ...
            throw new \including\Exception\Db($e->getMessage(), $e->getCode(), $e);
        }
    }
    ...
    public function selectAll($table, $columns, $where = array(), $sqlEnd = '')
    {
        ...
        $sql = 'SELECT ' . $columns . ' FROM ' . knTable($table);
    ...
    {
        $sql = 'SELECT ' . $column . ' FROM ' . knTable($table);
    ...
    public function insert($table, $row, $ignore = false)
    {
        ...
        $sql = "INSERT {$_ignore} INTO " . knTable($table) . " (";
        ...
        if (empty($params)) {
            $sql = "INSERT {$_ignore} INTO " . knTable($table) . " () VALUES()";
        }
    ...
    {
        $sql = "DELETE FROM " . knTable($table, false) . " WHERE ";
        $params = array();
    ...
    public function sqlMinAge($fieldName, $minAge, $unit='HOUR') {
        if (!in_array($unit, array('MINUTE', 'HOUR'))) {
            throw \including\Exception("Only 'MINUTE' or 'HOUR' are available as unit options.");
        }
