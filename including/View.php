...
class View
{
    ...
    public function __construct($file, $data = array())
    {
        ...
        $this->doctype = constant('\including\Response\Layout::' . $doctypeConstant);
    }
    ...
    public function render()
    {
        ...
        require($file); // file existence has been checked in knView function
    ...
    private static function checkData($data)
    {
        foreach ($data as $key => $value) {
            if (!preg_match('/^[a-zA-Z0-9_-]+$/', $key) || $key == '') {
                ...
                throw new \including\Exception\View("Incorrect view variable name '" . esc($key) . "' " . esc($source));
            }
        }
    }
