...
use including\Internal\Content\Model;
...
class WidgetController
{
    ...
    public function __construct($name, $pluginName, $core = false)
    {
        ...
        if ($this->core) {
            ...
            $this->widgetDir = 'including/Internal/' . $pluginName . '/' . Model::WIDGET_DIR . '/' . $this->name . '/';
        } else {
            $this->widgetDir = 'Plugin/' . $pluginName . '/' . Model::WIDGET_DIR . '/' . $this->name . '/';
        }

        $this->widgetAssetsDir = $this->widgetDir . \including\Application::ASSETS_DIR . '/';
    }
    ...
    /**
     * Check if the widget is native Kholifa CMS widget
     *
    ...
    {
        if ($this->core) {
            if (file_exists(knFile($this->widgetAssetsDir . 'icon.svg'))) {
                return knFileUrl($this->widgetAssetsDir . 'icon.svg');
            }
            if (file_exists(knFile($this->widgetAssetsDir . 'icon.png'))) {
                return knFileUrl($this->widgetAssetsDir . 'icon.png');
            }
        } else {
            if (file_exists(knFile($this->widgetAssetsDir . 'icon.svg'))) {
                return knFileUrl($this->widgetAssetsDir . 'icon.svg');
        ...
        return knFileUrl('including/Internal/Content/assets/img/iconWidget.svg');
    }
     ...
     * @throws \including\Exception\Content
     */
    public function getSkins()
    {
        ...
        $skinDir = knFile($this->widgetDir . self::SKIN_DIR . '/');


        if (!is_dir($skinDir)) {
            throw new \including\Exception\Content('Skins directory does not exist. ' . esc($skinDir));
        }
        ...
        if ($this->isCore()) {
            $themeViewsFolder = knThemeFile(
                \including\View::OVERRIDE_DIR . '/including/Internal/' . $this->pluginName . '/' . Model::WIDGET_DIR . '/' . $this->name . '/' . self::SKIN_DIR
            );
        } else {
            $themeViewsFolder = knThemeFile(
                \including\View::OVERRIDE_DIR . '/Plugin/' . $this->pluginName . '/' . Model::WIDGET_DIR . '/' . $this->name . '/' . self::SKIN_DIR
            );
        }
        ...
        foreach ($views as $viewKey => $view) {
            if ($this->isCore()) {
                $translation = __(ucfirst($viewKey), 'including-admin', false);
            } else {
        ...
        if (empty($skins)) {
            throw new \including\Exception\Content('No skins');
        }
     ...
     *
     * Use return new \including\Response\Json($jsonArray) to return JSON.
     *
    ...
    {
        $snippetDir = knFile($this->getWidgetDir() . \including\Internal\Content\Model::SNIPPET_DIR) . '/';
        ...
        foreach ($snippetFiles as $snippetFile) {
            if (is_file($snippetDir . $snippetFile) && substr($snippetFile, -4) == '.php') {
                $snippet .= knView($snippetDir . $snippetFile)->render();
            }
        }
    ...
    public function generateHtml($revisionId, $widgetId, $data, $skin)
    {
        ...
        try {
            if ($this->core) {
                $skinFile = 'including/Internal/' . $this->pluginName . '/' . Model::WIDGET_DIR . '/' . $this->name . '/' . self::SKIN_DIR . '/' . $skin . '.php';
            } else {
                $skinFile = 'Plugin/' . $this->pluginName . '/' . Model::WIDGET_DIR . '/' . $this->name . '/' . self::SKIN_DIR . '/' . $skin . '.php';
            }
            if (!is_file(knFile($skinFile)) && !is_file(knThemeFile(\including\View::OVERRIDE_DIR . '/' . $skinFile))) {
                $skin = 'default';
                if ($this->core) {
                    $skinFile = 'including/Internal/' . $this->pluginName . '/' . Model::WIDGET_DIR . '/' . $this->name . '/' . self::SKIN_DIR . '/' . $skin . '.php';
                } else {
            ...
            $answer = knView($skinFile, $data)->render();

        } catch (\including\Exception $e) {
            if (knIsManagementState()) {
                $answer = $e->getMessage() . "\n " . $e->getTraceAsString();
            } else {
                $answer = '';
            }
...
//            'attributes' => array(
//                'class' => 'knsMyMenu', //class to be added. Could be used in JS to bind actions on this button
//                'data-somedata' => json_encode('lorem knsum'), //data that later can be accessed in js. E.g. $('.knsMyMenu').first().data('somedata');
//                ...
//            )
//        );
//        return $answer;
