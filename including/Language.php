...
class Language
{
    ...
    {
        $row = knDb()->selectRow('language', '*', array('code' => $languageCode));
        if (!$row) {
            return null;
        }
    ...
    {
        if ($this->getId() == knContent()->getCurrentLanguage()->getId()) {
            return true;
        } else {
            return false;
        }
    ...
    {
        $link = knConfig()->baseUrl();

        if (knConfig()->get('rewritesDisabled')) {
            $link .= 'index.php/';
        }

        $link .= \including\ServiceLocator::content()->getLanguage($this->getId())->getUrlPath();
