...
class Config
{
    ...
    public function __construct($config, $server = null)
    {
        ...
        if (empty($this->config['baseUrl'])) {
            ...
            // Kholifa CMS from a subdirectory when using the built-in webserver
        ...
        if (empty($this->config['coreDir'])) {
            if ($this->isComposerCore()) {
                $this->config['coreDir'] = realpath(dirname(getcwd()) . '/vendor/Kholifa-CMS/kholifa-CMS');
            } else {
    ...
    public function theme()
    {
        if (!empty($this->config['theme'])) {
            ...
            return knStorage()->get('content', 'theme');
        }
    }

    public function adminLocale()
    {
        if (!empty($_COOKIE["knAdminLocale"])) {
            return $_COOKIE["knAdminLocale"];
        }
    ...
    {
        return is_dir(dirname(getcwd()) . '/vendor/kholifa-CMS/kholifa-CMS');
    }
    ...
    protected function getEnv() {
        $environment = getenv('content_ENV');
