...
class Page
{
    ...
    public function __construct($id)
    {
        if (is_int($id) || ctype_digit($id)) {
            ...
            $page = knDb()->selectRow('page', '*', array('id' => $id, 'isDeleted' => 0));
            if (!$page) {
                $id = esc($id);
                throw new \including\Exception("Page #" . esc($id) . " not found.");
            }
        } elseif (is_array($id)) {
            ...
            $page = knDb()->selectRow('page', '*', array('alias' => $id, 'isDeleted' => 0));
            if (!$page) {
                throw new \including\Exception("Page #" . esc($id) . " not found.");
            }
        }
     ...
     *
     * @param \including\Controller $controller
     */
    public function init(\including\Controller $controller)
    {
    ...
    public function getLink()
    {
        ...
        return knHomeUrl($this->getLanguageCode()) . $path;
    }
    ...
    {
        $curPage = knContent()->getCurrentPage();
        return $curPage && $this->getId() == knContent()->getCurrentPage()->getId();
    }
    ...
    public function isInCurrentBreadcrumb()
    {
        if ($this->inBreadcrumb === null) {
            $breadcrumb = knContent()->getBreadcrumb();
            $ids = array();
    ...
    public function getChildren($from = null, $till = null, $orderBy = 'pageOrder', $direction = 'ASC')
    {
        switch ($orderBy) {
            ...
            case 'deletedAt':
                ...
                break;
            default:
                throw new \including\Exception("getChildren can't accept " . esc($orderBy) . " as an order field.");
        }
    ...
    public function getLayout()
    {
        if ($this->layout) {
            ...
            $menu = knContent()->getPageMenu($this->id);
