...
class Form
{
    ...
    public function __construct()
    {
        ...
        $this->action = knConfig()->baseUrl();
        $this->attributes = array();
        $this->classes = array();
        $this->ajaxSubmit = true;
        $this->validate = true;
        $this->addClass('knsAjaxSubmit');
        if (knRoute()->isAdmin()) {
            $this->setEnvironment(self::ENVIRONMENT_ADMIN);
        } else {
            $this->addClass('knModuleForm');
    ...
    {
        $tokenField = new \including\Form\Field\Antispam();
     ...
     * Kholifa CMS tries to detect environment automatically based on current controller. You can set manually the right mode if needed.
     * @param $environment
     */
    public function setEnvironment($environment)
    {
        if ($environment == self::ENVIRONMENT_ADMIN) {
            $this->addClass('knsModuleFormAdmin');
            $this->removeClass('knModuleForm');
            $this->removeClass('knsModuleFormPublic');
        } else {
            $this->addClass('knsModuleFormPublic');
            $this->addClass('knModuleForm');
            $this->removeClass('knsModuleFormAdmin');
        }
    ...
    {
        $tokenField = new \including\Form\Field\Csrf();
     ...
     * @param \including\Form\Fieldset | string $fieldset
     */
    public function addFieldset($fieldset)
    {
        if (is_string($fieldset)) {
            $fieldset = new \including\Form\Fieldset($fieldset);
        }
     ...
     */
    public function addField(\including\Form\Field $field)
    {
     ...
     *
     * Set post method.
     * @param string $method Use \including\Form::METHOD_POST or \including\Form::METHOD_GET
     * @throws Exception
     */
     ...
     */
    public function render(\including\View $view = null)
    {
        if (!$view) {
            if ($this->getEnvironment() == self::ENVIRONMENT_ADMIN) {
                $view = knView('Form/adminView/form.php');
            } else {
                $view = knView('Form/publicView/form.php');
            }
        }
        $view->setVariables(
            array(
    ...
    public function setAjaxSubmit($ajaxSubmit)
    {
        if ($ajaxSubmit) {
            ...
            $this->removeClass('knsAjaxSubmit');
        }
    ...
    {
        if ($validate) {
            $this->addClass('knsValidate');
        } else {
            $this->removeClass('knsValidate');
        }
