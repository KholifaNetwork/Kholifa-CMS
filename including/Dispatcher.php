...
class Dispatcher
{
    ...
    {
        $this->eventDispatcher = new \including\Internal\Dispatcher\EventDispatcher();
        $this->jobDispatcher = new \including\Internal\Dispatcher\JobDispatcher();
        $this->filterDispatcher = new \including\Internal\Dispatcher\FilterDispatcher();
        $this->slotDispatcher = new \including\Internal\Dispatcher\SlotDispatcher();
    }
    ...
    {
        // Parse event files:
        $coreModules = \including\Internal\Plugins\Model::getModules();
        foreach ($coreModules as $module) {
            $this->bindPluginEvents($module, '\including\Internal');
        }
    ...
    private function bindPluginEventType($plugin, $type, $namespace)
    {
        ...
        foreach ($methods as $method) {
            ...
            } elseif (knConfig()->isDevelopmentEnvironment()) {
                throw new \including\Exception(esc("{$plugin}\\{$type}::{$method->getName()} must be static."));
            }
        }
        return null;
    }
